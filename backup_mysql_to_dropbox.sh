...
# Usage: ./backup_mysql.sh [/path/to/.env]   (defaults to same dir as script)
#
...
required_vars=(DB_HOST DB_PORT DB_NAME DB_USER DB_PASSWORD DB_CONTAINER DROPBOX_DIR BACKUP_DST BACKUP_PASSPHRASE_FILE)
...
# Pass the password via MYSQL_PWD env var instead of --password flag
