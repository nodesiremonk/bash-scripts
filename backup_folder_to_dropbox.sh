...
# Usage: ./backup_folder.sh [/path/to/.env]   (defaults to same dir as script)
#
...
while IFS='=' read -r key value; do
    ...
    [[ -z "$key" ]]               && continue
    case "$key" in
        BACKUP_SOURCES|DROPBOX_DIR|BACKUP_DST|BACKUP_PASSPHRASE_FILE)
...
required_vars=(BACKUP_SOURCES DROPBOX_DIR BACKUP_DST BACKUP_PASSPHRASE_FILE)
...
echo "[INFO] Validating source folders …"
...
# ── Compress ──────────────────────────────────────────────────────────────────
echo "[INFO] Compressing folders …"
# shellcheck disable=SC2086
if ! tar -czf "$ARCHIVE_FILE" $BACKUP_SOURCES; then
